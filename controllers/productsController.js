...
export const createProductController = async (req, res) => {
  try {
    const { name, price, description, category, color, imgLink } =
      req.fields;
    ...
    switch (true) {
      ...
      case !photo &&  !imgLink:
        return res.status(500).send({ message: "photo or Link is required" });
        break;
      case !category:
...
export const filterProductController = async (req, res) => {
  try {
    const { checked, radio,color } = req.body;
    let args = {};
...
export const updateProductController = async (req, res) => {
  try {
    const { name, description, price, category, imgLink, color } =
      req.fields;
    const { photo } = req.files;
    //validation
    switch (true) {
