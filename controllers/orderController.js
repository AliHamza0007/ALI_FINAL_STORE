    ...
    if (order) {
      async function main() {
        const info = await transporter.sendMail({
          ...
          subject: "ALI_FINAL_STORE Order", // Subject line
      ...
      async function user() {
        const info = await transporter.sendMail({
          from: "confortzoneuk@gmail.com", // list of sender
          to: `${req.body.userEmail}`, // receivers address
          // to: `hamzasarwer9@gmail.com`, // list of receivers

          subject: "ALI_FINAL_STORE Order Place", // Subject line
    ...
    if (order) {
      async function main() {
        const info = await transporter.sendMail({
          ...
          subject: "ALI_FINAL_STORE", // Subject line
    ...
    if (order) {
      async function main() {
        const info = await transporter.sendMail({
          ...
          html: `<h2>
     Your Order status is Updated by ALI_FINAL_STORE Admin</h2><h2>${orderUser.username} Check Your Status On ConfortZone Website</h2>`, // html body
        });
