...
const AdminOrders = () => {
  ...
  const getOrder = async () => {
    let result = await fetch(
      `${apiUrl}/api/v1/order/get-allorder`,
  ...
  const updateStatus = async (id, value) => {
    ...
    let result = await fetch(
      `${apiUrl}/api/v1/order/update-orderstatus/${id}`,
      {
        method: "PUT",
        headers: {
  ...
  const searchOrder = async (e) => {
    ...
    if (value) {
      let result = await fetch(
        `${apiUrl}/api/v1/order/search-order/${value}`,
        {
                            ...
                            <tbody className="table-info table-hover text-center align-items-center">
                              {o?.cartItems?.map((p) => (
                                <tr key={p.product}>
                                  <td>
                                    <img
                                      src={p.imgLink?p.imgLink:`${apiUrl}/api/v1/product/photo-product/${p.product}`}
