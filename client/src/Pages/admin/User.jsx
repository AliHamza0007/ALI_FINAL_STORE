...
const User = () => {
  ...
  const getUsers = async () => {
    let result = await fetch(`${apiUrl}/api/v1/auth/get-users`, {
      method: "GET",
  ...
  const removeUser = async (id) => {
    ...
    if (confirm) {
      let result = await fetch(`${apiUrl}/api/v1/auth/delete-user/${id}`, {
        method: "delete",
        headers: { Authorization: auth?.token },
      });
      result = await result.json();
  ...
  const searchUser = async (e) => {
    ...
    if (key) {
      let result = await fetch(`${apiUrl}/api/v1/auth/search-user/${key}`, {
        headers: { Authorization: auth?.token },
      });
      result = await result.json();
      if (result?.success) {
  ...
  async function getSingleUser(id) {
    let result = await fetch(`${apiUrl}/api/v1/auth/single-user/${id}`, {
      method: "get",
      headers: { Authorization: auth?.token },
    });
    result = await result.json();
  ...
  const updateUser = async () => {
    try {
      ...
      if (!name || !email) {
        ...
        let result = await fetch(`${apiUrl}/api/v1/auth/user-update/${id}`, {
          method: "put",
          headers: {
        ...
        <div className="row">
          <div className="col-md-12 col  ">
            <div className="text-center my-4">
              <RubberBand>
                <h1 className="text-primary">Manage User</h1>
              </RubberBand>

              <div className="row  py-1 ">
                <div className="col text-center">
                  <Fade right>{users?.length} Users Available</Fade>
                  <input
                    className=" form-control text-center m-1 px-3"
                    onChange={searchUser}
                    type="text"
